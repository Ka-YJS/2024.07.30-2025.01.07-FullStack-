...
/*	*POST,PUT,DELETE으로 전달하면 데이터들이 RequestBody로 전송됨
	-> GET으로 전달할 때는 RequestBody로 전송되지 않음
	-> localhost:9090/users/idCheck?userid='xx' */
	
	@PostMapping("idCheck")
//	public ResponseEntity<?> isIdDuplicate(@RequestParam("userId") UserDTO dto){ -> GET으로 데이터를 받을 때 사용함
	...
	@GetMapping("/name")
	//@RequestHeader("Authorization") :  HTTP요청헤더 값을 컨트롤러의 메서드에 주입하는데 사용되는 어노테이션
	  public ResponseEntity<?> getUserName(@RequestHeader("Authorization") String token){
	     //Bearer~~라고 나오므로 "Bearer " 제거
	     String actualToken = token.substring(7);
	       
	      // JWT에서 유저 id 추출
	      String userId = tokenProvider.validateAndeGetUserId(actualToken);
	       
	      //select * from users where userid = ' '; 에서 ' '안에 userId가 들어간다고 보면 됨
