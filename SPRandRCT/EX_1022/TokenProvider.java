...
//		return null;
	...
	public String create(Authentication authentication) {
		//OAuth2.0 인증을 통해 로그인한 사람의 정보를 나타냄
		ApplicationOAuth2User userPricipal = (ApplicationOAuth2User)authentication.getPrincipal();
		//Authentication : 어플리케이션내에서 사용자가 누구인지와 그 사용자가 권한을 가졌는지 여부를 나타내는 객체
		//Pricipal : 사용자를 나타내는 정보, 보통 사용자 ID또는 사용자 객체가 들어감
		...
		return Jwts.builder()
				.signWith(SignatureAlgorithm.HS512,SECRET_KEY)
				.setSubject(userPricipal.getName())//id가 반환됨
				.setIssuer("demo app") //토큰 발행 주체
				.setIssuedAt(new Date()) //토큰 발행 날짜
				.setExpiration(expiryDate) // exp
				.compact(); //토큰을 . 으로 구분된 하나의 문자열로 만들어준다
